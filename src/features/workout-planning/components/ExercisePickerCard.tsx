...
export function ExercisePickerCard({ item, pr, onAdd }: ExercisePickerCardProps) {
  ...
  return (
    <TouchableOpacity style={[styles.card, isAdded && styles.cardFeedbackActive]} activeOpacity={0.7} onPress={handlePress}>
      <View style={{ flex: 1, marginRight: 10 }}>
        ...
        <View style={styles.metaBadgeContainer}>
          <View style={[styles.metaBadge, isComposto ? styles.badgeComposto : styles.badgeIsolado]}>
            <Text style={[styles.metaBadgeText, { color: isComposto ? appTheme.colors.accent : appTheme.colors.textSecondary }]}>{item.mechanic.toUpperCase()}</Text>
          </View>
          <Text style={styles.subtleTargetText}>{item.equipment.toUpperCase()} • {item.muscleGroup.toUpperCase()}</Text>
        </View>
      </View>
      ...
      <View style={[styles.addIconCircle, isAdded && styles.addIconCircleSuccess]}>
        <Ionicons name={isAdded ? "checkmark" : "add"} size={16} color={isAdded ? appTheme.colors.textPrimary : "#FFF"} />
      </View>
    </TouchableOpacity>
  );
}
...
const styles = StyleSheet.create({
  ...
  prBadge: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "rgba(255, 159, 10, 0.1)", borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4 },
